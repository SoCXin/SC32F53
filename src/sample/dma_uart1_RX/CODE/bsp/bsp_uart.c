...
void UART_Config(void)
{
		...
		GPIO_InitStructInit(&GPIO_InitStructure);
		UART_StructInit(&UART_InitStructure);
	
		// 打开串口外设的时钟
		DEBUG_UART_APBxClkCmd(DEBUG_UART_CLK,ENABLE);
		
		GPIO_SetAFMode(DEBUG_UART_TX_GPIO_PORT,DEBUG_UART_TX_GPIO_PIN,DEBUG_UART_TX_GPIO_AFx);
		GPIO_SetAFMode(DEBUG_UART_RX_GPIO_PORT,DEBUG_UART_RX_GPIO_PIN,DEBUG_UART_RX_GPIO_AFx);
		...
		UART_FIFO_Cmd(DEBUG_UARTx,ENABLE);		//FIFO使能
		UART_FIFO_Config(DEBUG_UARTx,UART_RXFIFO_Half,UART_TXFIFO_SevenEighths);   //FIFO configuration

//		UART_EnableIRQ(DEBUG_UARTx,1);    //UART中断配置、使能
//		UART_ITCmd(DEBUG_UARTx,UART_IT_Receive,ENABLE);   // 开启Receive中断
		
		UART_DMACmd(DEBUG_UARTx,UART_DMAReq_RX,ENABLE);   //RX触发DMA使能
